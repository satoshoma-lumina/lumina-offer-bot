...
import requests # ★★★★★ 診断用にrequestsを追加 ★★★★★
...
def send_notification_email(subject, body):
    ...
    if not from_email or not api_key: return
...
def find_and_generate_offer(user_wishes):
    ...
    try:
        ...
        if not location: return None, None, "希望勤務地の位置情報を特定できませんでした。"
    ...
    # STEP 2: ライブラリを使わず、直接Gemini APIを呼び出す
    prompt_text = f"""
    ...
    try:
        ...
        if not api_key: return None, None, "GEMINI_API_KEYが設定されていません。"
        
        # 安定している v1 エンドポイントと gemini-pro モデルを直接指定
        url = f"https://generativelanguage.googleapis.com/v1/models/gemini-pro:generateContent?key={api_key}"
        headers = {"Content-Type": "application/json"}
        data = {"contents": [{"parts": [{"text": prompt_text}]}]}

        response = requests.post(url, headers=headers, json=data)
        response.raise_for_status() # 200番台以外のステータスコードはここでエラーを送出
        ...
        return None, None, f"AIサービスへの接続に失敗しました。"
    ...
    try:
        ...
        if not json_str_match: raise ValueError("AIの応答にJSONが含まれていません。")
    ...
    try:
        ...
        for cell in user_cells:
            ...
            if str(record_salon_id) == str(salon_id):
                ...
                break
        if row_to_update != -1:
            update_values = ['日程調整中', data['interviewMethod'], data['date1'], data['startTime1'], data['endTime1'], data['date2'], data['startTime2'], data['endTime2'], data['date3'], data['startTime3'], data['endTime3']]
